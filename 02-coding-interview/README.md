...
6. Click **Run** to execute JavaScript/TypeScript snippets inside the sandboxed iframe. The output appears below the editor. Other languages are for collaboration only because shipping full WebAssembly runtimes (for instance, Pyodide is ~10–15 MB compressed before initialization) would bloat the demo bundle (see `web/docs/runner.md`).
